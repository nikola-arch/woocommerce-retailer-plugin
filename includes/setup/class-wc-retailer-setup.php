...
class WC_Retailer_Setup extends WC_Retailer_Abstract_Loader {
                        ...
                        <form id="<?php echo esc_attr($config::RETAILER_SLUG); ?>-setup-form">
                            <table class="form-table">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" id="store_url" name="store_url" value="<?php echo esc_attr($store_url); ?>" class="regular-text" required />
                                        <p class="description">Your store's full URL.</p>
                                    </td>
                                </tr>
                            </table>
    ...
    public function handle_setup_submission() {
        ...
        if (empty($store_url)) {
            wp_send_json_error('Store URL is required');
            return;
        }
        ...
        if (is_wp_error($response)) {
            $debug_info = [
                'endpoint' => $config::SHOPWOO_BASE_URL . 'woocommerce/verify-store',
                'request_data' => [
                    'email' => $email,
                    'store_url' => $store_url,
        ...
        if ($response_code !== WC_Retailer_Constants::HTTP_OK) {
            $error_message = isset($body['error']) ? $body['error'] : 'Verification failed';
            $user_message = 'Verification failed! ' . (isset($body['message']) ? $body['message'] : $error_message);
    ...
    public function handle_email_verification() {
        ...
        $debug_info = [
            'endpoint' => $config::SHOPWOO_BASE_URL . 'woocommerce/verify-email',
            'response_code' => $response_code,
            'raw_response' => $raw_body,
            'parsed_response' => $body,
            'request_data' => [
        ...
        if ($response_code !== WC_Retailer_Constants::HTTP_OK) {
            $error_message = isset($body['error']) ? $body['error'] : 'Code verification failed';
            $user_message = 'Email verification failed! ' . (isset($body['message']) ? $body['message'] : $error_message);
            $response_data = ['message' => $user_message];
            if ($config::DEBUG_MODE) {
                $response_data['debug'] = $debug_info;
            }
    ...
    public function handle_complete_registration_ajax() {
        ...
        if ($response_code !== WC_Retailer_Constants::HTTP_OK) {
            $error_message = isset($body['error']) ? $body['error'] : (isset($body['message']) ? $body['message'] : 'Registration failed');

            $debug_info = [
                'endpoint' => $config::SHOPWOO_BASE_URL . 'woocommerce/register',
                'response_code' => $response_code,
                'raw_response' => $raw_body,
                'parsed_response' => $body,
                'request_data' => [
            ...
            $response_data = [
                'message' => 'Registration failed: ' . $error_message . ' (Code: ' . $response_code . ')'
            ];
    ...
    private function register_dropshipper($email, $store_url, $verification_id, $verification_code, $access_token) {
        ...
        return wp_remote_post($config::SHOPWOO_BASE_URL . 'woocommerce/register', [
    ...
    private function verify_woocommerce($email, $store_url, $retailer_uuid) {
        $config = self::config();
        return wp_remote_post($config::SHOPWOO_BASE_URL . 'woocommerce/verify-store', [
            'timeout' => WC_Retailer_Constants::API_TIMEOUT,
            'headers' => ['Content-Type' => 'application/json'],
            'body' => wp_json_encode([
                'email' => $email,
    ...
    private function verify_email_code($email, $verification_code, $verification_id, $access_token, $retailer_uuid) {
        ...
        return wp_remote_post($config::SHOPWOO_BASE_URL . 'woocommerce/verify-email', [
            'timeout' => WC_Retailer_Constants::API_TIMEOUT,
            'headers' => ['Content-Type' => 'application/json'],
            'body' => wp_json_encode([
                'retailer_uuid' => $retailer_uuid,
