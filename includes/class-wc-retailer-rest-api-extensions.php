...
//                array($this, 'handle_duplicate_sku_and_global_unique_id'),
//                10,
//                3
//            );
            add_action(
        ...
        public function embed_variations_in_response($response, $product, $request)
        {
            ...
            $variation_ids = $this->get_existing_variation_ids($product->get_id());
            ...
            if (!empty($variation_ids) && is_array($variation_ids)) {
                ...
                foreach ($variation_ids as $variation_id) {
                    ...
                    foreach ($variation->get_attributes() as $attribute_name => $attribute_value) {
                        ...
                        $id = $product_attribute_ids[$name] ?? 0;

                        if (!$id) {
                            $decodedName = wp_specialchars_decode($name);

                            if (isset($product_attribute_ids[$decodedName])) {
                                $name = $decodedName;
                                $id = $product_attribute_ids[$decodedName];
                            }
                        }
